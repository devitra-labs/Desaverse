...
import { 
  ...
  Environment, 
  Grid,
  useGLTF, // Import untuk load model
  Center,  // Import untuk menengahkan model
  Html     // Untuk loading state
} from '@react-three/drei';
import { Map, Home, Leaf, Zap, RefreshCw, Maximize2, Minimize2, Layers, Info, Activity } from 'lucide-react';
...
// --- KOMPONEN 3D SCENE (SUDAH DIGANTI DENGAN MODEL ASLI) ---
function VillageScene() {
  ...
  return (
    <group ref={groupRef}>
      ...
      <directionalLight position={[15, 20, 10]} intensity={2} castShadow />
      <pointLight position={[-15, 10, -10]} intensity={1.2} color="#10b981" />
      
      {/* Grid Lantai (Opsional, saya taruh agak bawah) */}
      <Grid position={[0, -0.6, 0]} args={[50, 50]} cellColor="#86efac" sectionColor="#22c55e" fadeDistance={50} cellSize={1} sectionSize={5} />

      {/* 2. Render Model Peta */}
      <Center position={[0, -0.5, 0]}>
        <primitive 
          object={scene} 
          // PENTING: Scale 0.001 agar peta muat di layar (tidak nge-zoom parah)
          scale={0.001} 
        />
      </Center>
    ...
    <div className={`bg-gradient-to-br from-slate-50 to-slate-100 min-h-screen ${isFullscreen ? 'fixed inset-0 z-50 p-0' : 'p-4 md:p-8'}`}>
      ...
      <div className={`bg-white rounded-2xl shadow-xl overflow-hidden border border-slate-100 flex flex-col ${isFullscreen ? 'h-full rounded-none border-none' : ''}`}>
        ...
        <div className={`relative w-full ${isFullscreen ? 'flex-1' : 'h-[400px] md:h-[600px]'}`}>
          {/* CAMERA: Posisi disesuaikan agar pas dengan scale 0.001 */}
          <Canvas shadows camera={{ position: [5, 5, 5], fov: 50, far: 1000 }}>
            <color attach="background" args={['#f0fdf4']} /> 
            <fog attach="fog" args={['#dcfce7', 20, 100]} />
